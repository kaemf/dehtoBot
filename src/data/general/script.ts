...
👉 до сплати ${priceSingle * countOfLessons} uah`,
    ...
    payment: {
      require: (lessonCount: number) => `👉🧾 Одержувач
ФОП Молоков Євгеній Альбертович
IBAN 
UA773220010000026004330103247
ЄДРПОУ 3706201794
Призначення платежу:
Оплата пакету на ${lessonCount} занять.\n
...
${zoom_link}`
  ...
  trialLesson: {
    ...
    goalLearnRequest: `яка ваша ціль вивчення мови? для чого її вчите?`,
...
${role !== 'guest'? `👉 <b>Кількість індивід. занять</b>: ${individual_count / 60} (${individual_count}хв)
<b>Викладач:</b> ${teacher}
...
${count > 0 ? '✅' : '❌'} <b>Залишок</b>: ${count} заняття (${clubPacket ? `${ConvertToPrice(clubPacket.toString()) ?? `(потрібно змінити, з попередньої версії ${clubPacket})`}uah` : 'Користувач не брав участі в клубах'})`
  ...
  operationWithGuest: (name: string, teacher: string, miro_link: string, countOfLessons: number, addToTrial?: boolean) => 
...
вибачте за незручності🫶🏼`
  ...
  deTask: {
      finalOrMore: `якщо це все, натисніть кнопку нижче, якщо ж ні - надішліть наступні матеріали`,
      finalOrMoreStudentEdition: 'якщо це все, натисніть на “відправити”, якщо ж ні - надішліть наступну відповідь'
  },
...
${countOfLessons > 0 ? '✅' : '❌'} <b>Залишок:</b> ${countOfLessons / 60} занять (${countOfLessons}хв)`,
      ...
      rescheduleLesson: (oldDayOfWeek: string, oldDay: string, oldMonth: string, oldTime: string, newDayOfWeek: string, newDay: string, newMonth: string, newTime: string, count: number) => 
...
✅ <b>Залишок:</b> ${count / 60} занять (${count}хв)`,
      ...
      rescheduleTrialLesson: (oldDayOfWeek: string, oldDay: string, oldMonth: string, oldTime: string, newDayOfWeek: string, newDay: string, newMonth: string, newTime: string) => 
...
✅ <b>Залишок:</b> ${count / 60} занять (${count}хв)\n\nпобачимося🍓 `,
      ...
      lessonComingNotification: (endUser: string, classType: string, minute: number, dayOfWeek: string, day: string, month: string, time: string, teacherName: string, count: number, link: string) =>
      `🌤 <b> ${classType === 'trial' ? 'Пробне заняття' : 'Заняття'} розпочнеться за ${minute}хв</b> 🌤\n
<b>Коли</b>: ${dayOfWeek}, ${day} ${month} о ${time} за Києвом🇺🇦
<b>${endUser === 'student' ? 'Викладач' : 'Студент'}</b>: ${teacherName}\n\n
${classType === 'trial' ? `👉 <b>Посилання:</b> ${link}\n\n` : ''}
...
✅ <b>Залишок:</b> ${count / 60} занять (${count}хв)`
    ...
    forAdmins: {
      ...
      rescheduleLesson: (studentName: string, username: string, number: string, teacherName: string, day: string, dayOfWeek: string, month: string, time: string, reason: string, count: number) => 
      `♻️️️ <b>Перенесення заняття</b> ♻️\n
<b>Студент:</b> ${studentName}
(@${username}); ${number}
<b>Викладач:</b> ${teacherName}\n
👉 заняття перенесено на ${dayOfWeek}, ${day} ${month} о ${time} за Києвом 🇺🇦
<b>Причина:</b> ${reason}\n
${count > 0 ? '✅' : '❌'} <b>Залишок:</b> ${count / 60} занять (${count}хв)`,

      rescheduleTrialLesson: (studentName: string, username: string, number: string, teacherName: string, day: string, dayOfWeek: string,  month: string, time: string, reason: string) => 
      `♻️️️ <b>Перенесення пробного заняття</b> ♻️\n
<b>Студент:</b> ${studentName}
(@${username}); ${number}
<b>Викладач:</b> ${teacherName}\n
...
👉 <b>посилання на зустріч:</b> ${zoom}`
  ...
  summerCamp: {
      entire: `раді вітати тебе на Літньому Кемпі від dehto!\n
ось декілька причин, чому ти не повинен пропустити цю подію:\n
⚡️ за літній період ти перейдеш на новий рівень вивчення мови без додаткових зусиль. Тільки вайб, тільки чіл, тільки корисна потрібна інформація, тільки досягнення твоєї особистої мети⚡ познайомишся з класними кемперами - заведеш нові знайомства⚡️ онлайн, триває 2 місяці (рівно 18 занять, 2 рази на тижень по 1,5год)  
⚡️ графік занять на 2 місяці вперед, який синхронізується з Google Calendar
⚡️ один кемп має 4-6 учасників відповідно до рівня знань мови 
⚡ ️розробляється мапа вивчення мови на цей період (теми, лексика, граматика), щоб було видно прогрес 
⚡ розіграш подарунків
⚡️ кожного місяця будуть розмовні клуби, які можна відвідати безкоштовно 
⚡️ у кожного студента є можливість 2 рази прийти на "індивідуальне продовження дня" (30хв індвідуальної роботи з викладачем на пояснення тем, які незрозумілі)  
⚡️ в кінці кемпу "екзамен" на перевірку знань 
⚡ сертифікат про закінчення навчання\n
якщо ти не хочеш закидувати вивчення німецької, але й на звичайне навчання часу і сил не вистачає, цей кемп точно для тебе!\n
<b>перейдемо до вибору рівня мови?</b>`,

      chooseLevel: `💁‍♂ щоб доєднати тебе до групи, обери свій точний рівень мови нижче\n
або, якщо вагаєшся з відповіддю - радимо пройти наш тест і тоді обирати рівень:
https://dehto.de/quizzes/test-na-riven/`,

      registerationComplet: (name: string) => `дякуємо, ${name}🫶🏽
ви успішно забронювали місце на літній кемп dehto!\n
очікуйте сповіщення від підтримки для ознайомлення з графіком, додання у групу та оплати ;)\n
дехто з турботою🍓`,

      notification: (name: string, username: string, number: string, level: string, dateRecord: string) =>
      `⛺️ Бажаючий на кемпи ⛺️\n
👉🏽${name} (@${username})
${number}
👉🏽Рівень: ${level}\n
дата: ${dateRecord}`
